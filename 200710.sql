...
SELECT *
FROM emp;
방금 입력한 9999번 사원번호를 갖는 사원의 deptno와 job 컬럼의 값을 SMITH사원의 deptno와 job 값으로 업데이트

UPDATE emp SET deptno = (SELECT deptno FROM emp WHERE ename = 'SMITH'), 
                job = (SELECT job FROM emp WHERE ename = 'SMITH')
WHERE empno = 9999;
==> UPDATE 쿼리1 실행할 때 안쪽 SELECT 쿼리가 2개가 포함됨 ==> 비효율적 고정된 값을 업데이트 하는게 아니라 다른 테이블에 있는 값을 통해서
    업데이트 할 때 비효율이 존재
...
DELETE 쿼리도 SELETE 쿼리 작성시 사용한 WHERE절과 동일
서브쿼리 사용 가능
...
선행 트랜잭션에서 읽은 데이터를 후행 트랜잭션에서 수정하지 못하게끔 막아, 선행 트랜잭션 안에서 항상 동일한
데이터가 조회되도록 보장하는 레벨

LEVEL 3 : SERLALIZABLE READ
후행 트랜잭션이 데이터를 입력, 수정 삭제하더라도 선행드랜잭션에서는 트랜잭션 시작 시점의 데이터가 보이도록 보장
